...
export function PostCreatePage() {
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<CreateRecruitmentPostInput>({
    resolver: zodResolver(createRecruitmentPostSchema),
    defaultValues: { teamId: teamId ?? '', description: '' },
  });

  const mutation = useMutation({
    mutationFn: async (input: CreateRecruitmentPostInput) => {
      const { data } = await api.post<RecruitmentPostDetail>('/recruitment/posts', input);
      return data;
    },
    ...
    onError: (err) => {
      const m =
        err instanceof AxiosError
          ...
          : null;
      setServerError(m ?? 'Không tạo được bài đăng');
    },
  });
      ...
      <main className="mx-auto max-w-3xl px-6 py-10 md:py-14">
        ...
        <form
          onSubmit={handleSubmit((values) => {
            ...
            mutation.mutate({
              ...values,
              teamId: teamId!,
              positionNeeded: values.positionNeeded?.trim() || undefined,
        ...
        >
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <Field label="Vị trí cần tuyển" error={errors.positionNeeded?.message}>
              <input
                type="text"
            ...
            <Field label="Khu vực ưu tiên" error={errors.region?.message}>
              <input
                type="text"
                {...register('region')}
